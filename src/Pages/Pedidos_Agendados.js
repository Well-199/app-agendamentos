...
import {View, Text, FlatList, RefreshControl, Alert, Image, TouchableOpacity, ScrollView, StyleSheet} from 'react-native';
...
const Pedidos_Agendados = ({navigation, route}) => {
    ...
    const start = async (pendingService) => {
        
        const req = await fetch(Api.url+"/api/agendado-pendente/"+entry.id, {
            method: "PUT",
            body: JSON.stringify(),
    ...
    const pendingService = async () => {
        /* GET ATUALIZA O STATUS E NAVEGA COM PARAMETROS ATUALIZADOS
        const tkn = await AsyncStorage.getItem('token');
    ...
    const callback = () => {
        ...
        if(entry.data <= date){
            start(pendingService);
            Alert.alert(
                "Serviço Iniciado",
                "Esse serviço permanecerá pendente até recebermos todas as imagens, para isso use o botão “Finalizar Serviço”.",
                [{ text: "OK", onPress: () => console.log("OK")}]
            );
        }
        else{
            alert(`Esse Serviço só pode ser iniciado em ${moment(entry.data).format("DD/MM/YYYY")}`);    
        }        
    };

    const connected = async (callback) => {
    ...
    return(
        <View style={styles.container}>
            ...
            <Header navigation={navigation} title="Serviço Agendado"></Header>

            <View style={{flexDirection: "row", justifyContent: "center"}}>
                <TouchableOpacity 
                    style={entry.data <= date ? styles.button2 : styles.button3} 
                    onPress={() => connected(callback)}>
                    <Text style={styles.button1Text}>Iniciar Serviço</Text>
                </TouchableOpacity>
            </View>
            ...
            <View style={{marginBottom: 100}}>
                ...
                <View style={{flexDirection: "row", justifyContent: "center"}}>
                    ...
                    onPress={() => navigation.navigate("Contato", {entry: entry, tela: "Pedidos_Agendados"})}>
...
export default Pedidos_Agendados;